...
3. note 只能写以下两类，随你选择：
   (a) **为什么这个半天这么排**（1句话 20-40 字）：例「上午访古迹人少，顺路去附近餐厅」、「灯光夜景适合晚上，走完饭后散步可达」。推荐优先写这类，让用户明白排期逻辑。
   (b) **纯提示**：出发时间 / 交通 / 预订提醒 / 天气 / 顺序。
   两类都不能出现新地点/餐厅/活动名称；只能引用 items[].name 中已存在的名字。如果换句些都写不出来，note = "" 即可。
...
6. 【地理顺路】同一个 slot 内的 items 必须在地理上彼此靠近，可步行或短途车程可达（< 5km / < 15 分钟车程）；同一天内不同 slot 之间也要按顺路推进（如东区→南区，不要东区→西郊→东区这种两头跑）。远距离的地点（如主城区 vs 郊区湿地/古镇）必须拆到不同的天或至少不同 slot。疑问时以"常识上同一个片区/街区/景区"为准，不确定则拆开。
